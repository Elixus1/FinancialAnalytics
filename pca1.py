...
X = df[["DTI", "FOIR", "LTV", "Age", "OldEmi"]].to_numpy()
...
plt.ylabel("Density")
plt.legend()
plt.savefig("pictures/pca.pdf", bbox_inches="tight")
plt.show()
...
plt.grid(True)
plt.savefig("pictures/screeplot.pdf", bbox_inches="tight")
plt.show()

features = ["DTI", "FOIR", "LTV", "Age", "OldEmi"]
pcs = ["PC1", "PC2", "PC3", "PC4", "PC5"]

loadings = pd.DataFrame(eigenvectors, index=features, columns=pcs)
print(loadings)
...
plt.savefig("pictures/loading_pca.pdf", bbox_inches="tight")
plt.show()
...
colors = {"Approve": "tab:blue", "Reject": "tab:red"}
...
plt.legend()
plt.grid(True)
plt.savefig("pictures/pc1vspc2.pdf", bbox_inches="tight")
plt.show()
